<script lang="ts">
	...
	const command = 'curl -fsSL https://bollyai.dev/install.sh | bash';
...
<section id="install" class="install"><div class="section-shell"><div class="install-grid"><Reveal><div><p class="eyebrow">Native installation</p><h2 class="section-title">A home for Bolly,<br />on hardware you own.</h2><p class="section-copy">Install the server on macOS or Linux, then download the desktop app for every computer you want to connect.</p><div class="facts"><span>OPEN SOURCE</span><span>MIT LICENSED</span><span>BYOK</span></div></div></Reveal><Reveal delay={100}><div class="terminal"><div class="terminal-head"><span>TERMINAL · INSTALL SERVER</span><i>macOS / Linux</i></div><div class="command"><span>$</span><code>{command}</code><button onclick={copyCommand} aria-label="Copy install command">{copied ? 'COPIED' : 'COPY'}</button></div><p class="copy-status" aria-live="polite" aria-atomic="true">{copyStatus}</p><div class="output"><p><b>01</b> Downloads the native server</p><p><b>02</b> Creates the local service</p><p><b>03</b> Opens onboarding at localhost:26559</p></div><a class="release" href="https://github.com/triangle-int/bolly/releases" target="_blank" rel="noopener"><span>DESKTOP APPS</span><strong>Download for macOS, Windows, or Linux</strong><i>↗</i></a></div></Reveal></div></div></section>
<style>
